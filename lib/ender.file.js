var fs = require('fs')
  ...
  , FILE = {
    ...
    , flattenDependencyTree: function (tree, uniques, callback) {
        ...
        for (var k in tree) {
          ...
          if (tree[k]) {
            ...
            uniques = uniques.concat(flattened);
          }
    ...
    , processPackages: function (_packages, _isAsync, context, callback) {
          ...
          FILE.flattenDependencyTree(tree, null, function (packages) {
            packages.forEach(function (name) {
              ...
              fs.readFile(location, 'utf-8', function (err, data) {
                ...
                FILE.constructSource(isAsync, _async, packagePath, packageJSON.main, function (result) {
                  if (!isAsync) {
                    source = result;
                  } else {
                    _async = result;
    ...
    , constructSource: function(isAsync, _async, packagePath, filePaths, callback) {
        var result = [];
