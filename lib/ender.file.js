var fs = require('fs')
  ...
  , FILE = {
    ...
    , getFileDependencies: function (relFilePaths, install, callback) {
        if (!relFilePaths.length) return callback && callback([]);
        var i = 0, l = relFilePaths.length, names = [], paths = [];

        function analyzerCallback (err, _names, _paths) {
          if (err) return console.log(err.message);
          names = names.concat(_names.reverse());
          paths = paths.concat(_paths.reverse());
          if (++i == l) {
            callback(UTIL.accumulate(paths, names));
          } else {
            analyzeNext(i)
          }
        }
        ...
        function analyzeNext(i) {
          ANALYZER(relFilePaths[i], null, install, analyzerCallback);
        }
    ...
    , processPackages: function (rawPackages, context, options, callback) {
        ...
        NPM.getNPMPackages(rawPackages, function (npmPackages, relFilePaths) {
          FILE.getFileDependencies(relFilePaths, true, function (__deps) {
