...
module.exports = {
    ...
    process: function (cmd, callback) {
      ...
      // Default options come from local preferences
      var prefs = ENDER.util.localPreferences()
      for (var pref in prefs) {
        options[pref] = prefs[pref]
      }
      if (typeof options.output == 'undefined' && typeof options.use != 'undefined') {
        options.output = options.use
      } else if (typeof options.output != 'undefined' && typeof options.output == 'undefined') {
        options.use = options.output
      }

      // Override default options using CLI arguments
