  ...
  API = {
    ...
    , add: function (packages, options, callback) {
        ...
        ENDER.get.buildHistory(options.use, function (cmd) {
          ENDER.cmd.process(cmd, function (type, _modules, _options) {
            ...
            ENDER.npm.getNPMPackages(_modules, function (npmPackages, relFilePaths) {
              ENDER.file.constructDependencyTree(npmPackages, 'node_modules', function (tree) {
                ENDER.file.flattenDependencyTree(tree, null, function (__modules, __uniquePackageNames) {
                  ENDER.file.validatePaths(__modules, __uniquePackageNames, function (modules, uniquePackageNames) {
                    var activePackages = uniquePackageNames.concat(ENDER.get.special(options)).concat(relFilePaths)
                      , queue = ENDER.util.unique(ENDER.util.reject(packages, activePackages));
                    if (!queue.length) {
                      console.log('specified packages already installed.');
                      process.exit();
                      return;
                    } else {
                      context = [type, ENDER.util.unique(ENDER.cmd.normalize(_modules.concat(queue))).join(' ')].join(' ') + (options.context || '');
                      if (_options.modules) context += ' --modules';
                      if (_options.output) context += ' --output ' + _options.output;
    ...
    , info: function (packages, options) {
        ...
        ENDER.get.buildHistory(options.use, function (cmd) {
          ENDER.cmd.process(cmd, function(type, _modules, _options) {
            options = ENDER.util.merge(_options, options);
            _modules = ENDER.util.unique(_modules);
            ENDER.npm.getNPMPackages(_modules, function (npmPackages, relFilePaths) {
              ENDER.file.getFileDependencies(relFilePaths, false, context, function (pathTree) {
                var depsPathTree = {}
                  , pkeys = Object.keys(pathTree).reverse();
                pkeys[0] && (depsPathTree[pkeys[0]] = pkeys.length > 1 ? {} : 0);
                for (var i = 1, l = pkeys.length; i < l; i++) {
                  depsPathTree[pkeys[0]][pkeys[i]] = 0;
                }
                ENDER.file.constructDependencyTree(npmPackages, 'node_modules', function (tree) {
                  ENDER.file.enderSize(options.use, function (size) {
                    ...
                    ENDER.file.prettyPrintDependencies(tree, depsPathTree);
                  });
                });
              });
            });
