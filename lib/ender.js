  ...
  API = {
        ...
        ENDER.get.buildHistory(function (cmd) {
          ENDER.cmd.process(cmd, function (type, _modules) {
            ...
            if (!queue.length) {
              ...
              context = [type, ENDER.cmd.normalize(modules.concat(queue)).join(' ')].join(' ');
              ENDER.npm.install(queue, function (err, out, stderr) {
                ...
                ENDER.file.assemble(modules.concat(queue), type == '-a' || type == 'async', context, function (source) {
                  var built = 0;
    ...
    , remove: function (packages) {
        ...
        ENDER.get.buildHistory(function (cmd) {
          ENDER.cmd.process(cmd, function(type, _modules) {
            var modules = ENDER.util.unique(ENDER.get.special(_modules))
              , queue = ENDER.util.reject(ENDER.util.unique(ENDER.util.keep(packages, modules)), ['ender-js']);
            if (!queue.length) {
              return console.log('Nothing to uninstall.');
            } else {
              ENDER.npm.uninstall(queue);
              modules = ENDER.util.reject(modules, queue);
              context = [type, ENDER.cmd.normalize(modules).join(' ')].join(' ');
            ...
            for (var i = 0, l = args.length; i < l; i++) {
              ENDER.npm.desc(args[i], function (description) {
                ...
                if (packages.length == args.length) {
                  ENDER.get.activeFileSize(function (size) {
