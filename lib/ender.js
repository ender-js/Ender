var colors = require('colors')
  ...
  API = {
    ...
    , add: function (packages, options, callback) {
        ...
        ENDER.get.buildHistory(options.use, function (cmd) {
          ENDER.cmd.process(cmd, function (type, _modules, _options) {
            ...
            ENDER.file.constructDependencyTree(ENDER.util.unique(ENDER.get.special(options).concat(_modules)), 'node_modules', function (tree) {
    ...
    , compile: function (files) {
        ...
        ENDER.closure.compile(files, function () {
          fs.readFile('./ender-app.js', function (er, out) {
            ENDER.file.gzip(out, function (data) {
              var size = (Math.round((data.length/1024) * 10) / 10) + 'kb';
              console.log('Success! Your compiled source is ' + (size).cyan + ' and available at ./ender-app.js'.green);
            });
          });
        });
      }
