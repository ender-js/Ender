        ...
        ENDER.get.buildHistory(function (cmd) {
          ENDER.cmd.process(cmd, function (type, _modules) {
            ENDER.file.constructDependencyTree(ENDER.util.unique(ENDER.get.special(_modules)), 'node_modules', function (tree) {
              ENDER.file.flattenDependencyTree(tree, null, function (modules, uniquePackageNames) {
                ...
                if (!queue.length) {
                  ...
                  if (ENDER.get.buildType(type) == 'just') {
                    API.just(_modules.concat(queue));
                  } else {
                    ENDER.npm.install(queue, function (err, out, stderr) {
                      ...
                      ENDER.file.assemble(_modules.concat(queue), ENDER.get.buildType(type) == 'async', context, function (source) {
